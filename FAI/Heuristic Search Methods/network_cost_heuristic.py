...
class index_heuristic_network():
    '''
    '''
    def __init__(self,N, branching_factor, connection_ratio = 2,  seed = 0, cost_range = np.arange(1,6), cost_freq = None):
        ...
        cost_matrix[np.tril_indices(N, k = -1)] = 0
        costs = choice(cost_range, int(cost_matrix.sum()), p = cost_freq)
        cost_matrix[cost_matrix == 1] = costs
    ...
    def check_connection(self, node0, node1):
        return ~np.isnan(self.cost_matrix[node0, node1])
